...
def test_docker_compose_binds_web_ui_to_loopback_by_default():
    ...
    assert '"127.0.0.1:${APP_PORT:-7000}:7000"' in compose
    assert (
        '"${ODYSSEUS_TAILSCALE_IP:?ODYSSEUS_TAILSCALE_IP is required}:'
        '${APP_PORT:-7000}:7000"'
    ) in compose
    assert "${APP_BIND:-127.0.0.1}:${APP_PORT:-7000}:7000" not in compose
